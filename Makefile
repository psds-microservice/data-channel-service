.PHONY: help build run run-dev clean tidy vet fmt health-check proto proto-build proto-generate proto-generate-local proto-generate-docker proto-openapi install-deps update migrate docker-build docker-compose-up docker-compose-down

APP_NAME = data-channel-service
CMD_PATH = ./cmd/data-channel-service
BIN_DIR = bin
PORT = 8093
PROTOC_IMAGE = local/protoc-go:latest
PROTO_ROOT = pkg/data_channel_service
PROTO_FILE = data_channel.proto
GEN_DIR = pkg/gen/data_channel_service
GO_MODULE = github.com/psds-microservice/data-channel-service
OPENAPI_OUT = api
.DEFAULT_GOAL := help

help:
	@echo "data-channel-service"
	...
	@echo "  make install-deps / update / migrate"
